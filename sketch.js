...
function preload()
{
  ...
  zapper1image= loadImage("zapper3.png");
  zapper2image = loadImage("zapper2.png");
  zapper3image = loadImage("zapper1.png")
...
function draw() {
  ...
  if(gameState==="play")
    ...
    if(touches.length > 0||keyIsDown(UP_ARROW)/* && keyDown("space")*/)
     {
      flag = true;
     ...
     if(mousePressedOver(reset))
        {
         ...
         petrolGrp.destroyEach(0);
          ...
          heart.visible = true;
          heart2.visible = true;
          backgrounds.changeAnimation("bg",backgroundimage);
        }
...
function zapper3()
{
  if(frameCount%90 === 0)
    {
      var zapper3 = createSprite(width,Math.round(random(height-550,height-200)));
